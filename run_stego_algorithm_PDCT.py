...
NUM_CHANNELS = 3
COVER_IMAGE_FILEPATH  = "/content/drive/MyDrive/Steg_new/Exp_Images/dog_on_sofa.jpg" # Choose your cover image (PNG)
STEGO_IMAGE_FILEPATH  = "/content/drive/MyDrive/Steg_new/Stego Image/stego_dog_image.jpg"
SECRET_MESSAGE_STRING = "Harry Potter is a series of seven fantasy novels written by British author J. K.Rowling. "
alpha = 1.09
...
for chan_index in range(NUM_CHANNELS):
    ...
    quantizedDCT = [fractional_matrix_power(q_dct_Block,alpha) for q_dct_Block in dct_blocks]
    
    # QUANTIZATION STAGE
    dct_quants = [np.around(np.divide(item, img.JPEG_STD_LUM_QUANT_TABLE)) for item in quantizedDCT]
    ...
    ip_dct = [fractional_matrix_power(q_dct_Block,(1/alpha)) for q_dct_Block in dct_dequants]
    ip_dct = [np.float32(dct_Block) for dct_Block in ip_dct]
    idct_blocks = [cv2.idct(block) for block in ip_dct]
